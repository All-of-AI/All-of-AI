...
from model.bert_classifier import modeling
from model.bert_classifier import optimization
from model.bert_classifier import tokenization
...
class InputExample(object):
    ...
    def __init__(self, guid, text_a, text_b=None, label=None):
        ...
        """
        self.guid = guid  # train+id, valid+od
        self.text_a = text_a
        self.text_b = text_b
        self.label = label
...
class Baidu_95_Multi_Label_Classification_Processor(DataProcessor):
    ...
    def create_example(lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append(InputExample(guid=guid, text_a=text_str, text_b=None, label=predicate_label_str))
        return examples
...
def file_based_input_fn_builder(input_file, seq_length, label_length, is_training, drop_remainder):
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    with tf.variable_scope("loss"):
        ...
        logits_wx = tf.matmul(output_layer, output_weights, transpose_b=True)  # output_weights转置
...
def main(_):
    ...
    # 加载bert模型配置文件
    # /model/bert/pretrained_model/chinese_L-12_H-768_A-12/bert_config.json
