...
(4) **容错性**：自然语言具有很高的容错性，哪怕一句话错得再离谱，人们还是可以猜出含义；编程语言的拼写必须保证绝对正确，语法必须保证绝对规范。
...
自然语言处理系统的输入源一共有三个，即语音、图像和文本。其中文本占主要地位，语音和图像受制于存储容量和传输速度的限制，它们的信息总量不如文本多。通常进行**语音识别(speech recognition)**将语音转化为文本，使用**光学字符识别(optical character recognition, OCR)**将图像转化为文本。
...
**句法分析(syntactic parsing)**将零散的词汇信息进行分析，得到**词语之间的关系**。问答系统、搜索引擎和机器翻译等系统中，句法分析都是非常重要的。
...
(2) **形式规则系统(rule system)**：形式规则系统中重要的模型有正则语法、正则关系、上下文无关语法、特征增益语法以及与这些语法相应的概率变体。
...
在正式处理任何一段文本之前，通常需要对文本进行规范化处理。以下三个任务是在任何规范化处理中经常被应用的过程：(1) **分词(tokenizing/segmenting words)**；(2) **规范文本格式(normalizingword formats)**；(3) **句子切分(segmenting sentences)**。
...
<img src="images/image-20200430090758651.png" style="zoom:50%;" />
...
<img src="images/image-20200430103137228.png" style="zoom:40%;" />

(2) 第二步，“(r, _)”这一字符对出现的频率最高，将其合并后加入vocabulary中：

<img src="images/image-20200430103249059.png" style="zoom:40%;" />

(3) 第三步，“(e, r_)”这一字符对出现的频率最高，将其合并后加入vocabulary中：

<img src="images/image-20200430103416331.png" style="zoom:40%;" />

(4) 第四步，以此类推：

<img src="images/image-20200430103708140.png" style="zoom:40%;" />

(5) 继续合并，会得到如下结果：

![image-20200430103739915](images/image-20200430103739915.png)
...
![image-20200430091820599](images/image-20200430091820599.png)
...
<img src="images/image-20200430091333212.png" style="zoom:40%;" />

(3) **语义学(semantics)**：传统NLP通常使用lambda演算，即精巧设计的函数来表达语义；在deep NLP中依然是**将每个语义单位用向量来进行表示**(应用：情感分析)。

<img src="images/image-20200430091419840.png" style="zoom:40%;" />
...
![image-20200430091527847](images/image-20200430091527847.png)

### 参考资料

[1] Dan Jurafsky, H. Martin. Speech and Language Processing(3rd ed. draft).

[2] 何晗. 自然语言处理入门.  北京: 人民邮电出版社, 2019.

[3] https://www.jb51.net/tools/regexsc.htm
